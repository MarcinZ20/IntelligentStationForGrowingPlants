...
static struct gatts_profile_inst gl_profile_tab_server[SERVER_PROFILE_NUM] = {
        [PROFILE_A_APP_ID] = {
...
char* uint8ToString(const uint8_t* data, size_t length) {
...
static void gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    case ESP_GAP_BLE_SCAN_RSP_DATA_RAW_SET_COMPLETE_EVT:
        ...
        case ESP_GAP_BLE_ADV_START_COMPLETE_EVT:
            ...
            ESP_LOGE(GATTS_TAG, "Advertisign started successfully!");
            break;
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    switch (event) {
        case ESP_GATTS_REG_EVT:
            ESP_LOGI(GATTS_TAG, "REGISTER_APP_EVT_A, status %d, app_id %d\n", param->reg.status, param->reg.app_id);
        ...
        case ESP_GATTS_WRITE_EVT: {
            ESP_LOGI(GATTS_TAG, "GATT_WRITE_EVT_A, conn_id %d, trans_id %" PRIu32 ", handle %d", param->write.conn_id, param->write.trans_id, param->write.handle);
            if (!param->write.is_prep){
                ...
                if (gl_profile_tab_server[PROFILE_A_APP_ID].descr_handle == param->write.handle && param->write.len == 2){
                    ...
                    if (descr_value == 0x0001){
                        if (a_property & ESP_GATT_CHAR_PROP_BIT_NOTIFY){
                            ...
                            esp_ble_gatts_send_indicate(gatts_if, param->write.conn_id, gl_profile_tab_server[PROFILE_A_APP_ID].char_handle,
        ...
        case ESP_GATTS_DISCONNECT_EVT:
            ESP_LOGI(GATTS_TAG, "ESP_GATTS_DISCONNECT_EVT, disconnect reason 0x%x", param->disconnect.conn_id);
            esp_ble_gap_start_advertising(&adv_params);
            break;
...
static void gatts_profile_b_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    switch (event) {
        case ESP_GATTS_REG_EVT:
            ESP_LOGI(GATTS_TAG, "REGISTER_APP_EVT_B, status %d, app_id %d", param->reg.status, param->reg.app_id);
        ...
        case ESP_GATTS_WRITE_EVT: {
            ESP_LOGI(GATTS_TAG, "GATT_WRITE_EVT_B, conn_id %d, trans_id %" PRIu32 ", handle %d", param->write.conn_id, param->write.trans_id, param->write.handle);
            if (!param->write.is_prep){
                ...
                if (gl_profile_tab_server[PROFILE_B_APP_ID].descr_handle == param->write.handle && param->write.len == 2){
                    ...
                    if (descr_value == 0x0001){
                        if (b_property & ESP_GATT_CHAR_PROP_BIT_NOTIFY){
                            ...
                            //the size of notify_data[] need less than MTU size
                            esp_ble_gatts_send_indicate(gatts_if, param->write.conn_id, gl_profile_tab_server[PROFILE_B_APP_ID].char_handle,
                                                        sizeof(notify_data), notify_data, false);
                        }
                    }else if (descr_value == 0x0002){
...
static void gatts_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    do {
        int idx;
        for (idx = 0; idx < SERVER_PROFILE_NUM; idx++) {
