...
mongoose.connect(`mongodb+srv://${process.env.MONGO_USER}:${process.env.MONGO_PASS}@cluster0.cugn5q3.mongodb.net/iot`)
...
const isPathExcluded = (req) => {
  ...
  const excluded = excludedPaths.find(
    (excludedPath) =>
      excludedPath.path === req.path && (excludedPath.method === req.method || !excludedPath.method)
  )
...
app.post('/api/login', async (req, res) => {
  ...
  const hashedAccess = bcrypt.hashSync(accessToken.slice(-20), parseInt(process.env.TOKEN_SALT_ITER, 10)) 
  const hashedRefresh = bcrypt.hashSync(refreshToken.slice(-20), parseInt(process.env.TOKEN_SALT_ITER, 10))
...
app.post('/api/refresh-token', async (req, res) => {
  ...
  jwt.verify(refreshToken, process.env.JWT_SECRET_REFRESH, async (err, decoded) => {
    ...
    if (!bcrypt.compareSync(refreshToken.slice(-20), activeTokens[decoded.id][1]))
      return res.status(401).json({ error: 'Unauthorized'})
...
app.put('/api/devices/history', async (req, res) => {
  try {
    const { deviceId, humidity, temperature, light_intensity, timestamp } = req.body 
    ...
    if (!existingUser)
      return res.status(401).json({ error: 'Device not registered for any user' })

    if (!deviceId || !timestamp) 
      return res.status(400).json({ error: 'deviceId and timestamp are required fields' }) 
    ...
    return res.json({ message: 'Reading added successfully', reading: newReading }) 
  } catch (error) {
    console.error(error) 
  ...
  if (token) {
    jwt.verify(token, process.env.JWT_SECRET_ACCESS, async (err, decoded) => {
      ...
      if (!bcrypt.compareSync(token.slice(-20), activeTokens[decoded.id][0])) {
        return res.status(403).json({ error: 'Unauthorized - Token no longer active' })
      }
...
app.get('/api/userId', async (req, res) => {
  ...
  if (user){
    ...
    return res.status(403).json({ error: 'Access denied. You must be logged in' })
})
...
app.get('/api/devices/:id/current', async (req, res) => {
  ...
  if (found)
    ...
    return res.status(404).json({ error: 'No such device found for user' })
})
...
app.post('/api/devices', async (req, res) => {
  ...
  await User.findByIdAndUpdate(req.auth.id, { devicesIds: newDevicesIds }, { new: true })
  return res.json({ message: 'Successfuly updated devices', devices: newDevicesIds })
})
...
app.put('/api/devices', async (req, res) => {
  ...
  return res.json({ message: 'Successfuly updated devices', user: updatedUser })
})
...
app.delete('/api/devices', async (req, res) => {
  ...
  return res.json({ message: 'Removed all devices from user', user: updatedUser })
})
...
app.delete('/api/devices/:id', async (req, res) => {
  ...
  if (index !== -1) {
    ...
    return res.json({ message: 'Succesfully removed device', user: updatedUser })
  } else {
